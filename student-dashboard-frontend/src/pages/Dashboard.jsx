        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 flex flex-col w-full">
            ...
            <div className="w-full">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:space-x-10">
                {indicators.slice(0, showMore ? indicators.length : 4).map((indicator, index) => (
                  <PerformanceBox indicator={indicator} handlePercentage={handlePercentage} key={index} />
                ))}
              </div>
          ...
          <div className="flex flex-col sm:flex-row  md:items-start lg:flex-col gap-6 p-3">
            <Calendar
              ...
              classNames={{
                day_selected: "bg-gradient-to-r from-[#8c55d3] to-black text-white",
              ...
              {assignments.map((assignment, index) => (
                <div key={index} className="flex justify-between items-center mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-3 h-3 rounded-full ${assignment.status === "completed" ? "bg-lime-400" : "bg-red-500"}`}></div>
                    <div>
                      <p className="text-sm">{assignment.subject}</p>
                      <div className="flex items-center gap-2">
                        <Timer className="w-3 h-3 text-purple-500" />
                        <p className="text-xs">{formatDate(assignment.dueDate)}</p>
                      </div>
                    </div>
                  </div>
                  <button
