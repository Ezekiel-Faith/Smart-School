    ...
    >
      ...
      <div className='px-10 md:px-4 py-4 md:py-15 lg:py-4'>
        <nav className='flex flex-col flex-1'>
          {MAIN_NAV.map(({ icon: Icon, label, to }) => (
            <NavLink
              key={label}
              to={to}
              onClick={toggleSidebar} // CHANGE: auto close on click (like right version)
              className={({ isActive }) =>
                `flex items-center gap-3 p-2 md:p-4 lg:p-2
        ...
        <div className='flex flex-col gap-0 mt-auto md:mt-8 lg:mt-6 xl:mt-40'>
          <p className='px-2 py-1 md:px-4 lg:px-2 md:text-2xl lg:text-sm'>
            SETTINGS
          </p>
          ...
          {bottomItems.map(({ icon: Icon, label, extra }) => (
            <a
              ...
              className={`flex items-center gap-2 
                          px-2 py-1 md:px-5 md:py-2 lg:px-2 lg:py-1
                          text-lg md:text-2xl lg:text-sm
                          hover:bg-purple-600 ${extra || ''}`}
            >
              <Icon size={15} className='md:w-7 md:h-7 lg:w-4 lg:h-4 pl-2' />
              {label}
            </a>
          ))}
        </div>
